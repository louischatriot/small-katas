...
matrix = [list(map(int, l.split())) for l in s.split('\n')[1:-1]]
...
for l in itertools.chain(matrix, transpose(matrix), diagonals(matrix)):
    if len(l) < b:
        continue

    res = max(res, max([math.prod(l[i:i+b]) for i in range(0, len(l) - b + 1)]))
